...
public class OpenNoteCameraView extends JavaCameraView implements PictureCallback {
  ...
  public interface OnProcessingListener {
    void onProcessingChange(String path);
  }
  ...
  public void documentDetected(boolean detected) {
    mThis.processingListener.onProcessingChange(detected ? "detected" : "searching");
  }
  ...
  public void waitSpinnerVisible() {
    mActivity.runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        mThis.processingListener.onProcessingChange("processing");
      }
    });
  }
  ...
  private String saveToDirectory(Mat doc) {
    ...
    String folderDir = this.saveOnDevice ? Environment.getExternalStorageDirectory().toString()
  ...
  public void saveDocument(ScannedDocument scannedDocument) {
    ...
    if (this.listener != null) {
      ...
      data.putString("capturedImage", "file://" + initialFileName);
